...
const Sidebar = ({ onNewChat, onSelectConversation }: SidebarProps) => {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('conversationUpdated', handler);
  }, [user]);
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-transparent">
        {!isCollapsed && (
          <>
            ...
            {loading && <div className="px-2 py-3 text-xs text-gray-400">{getTranslation('loading')}</div>}

            {!loading && conversations.length === 0 && (
              <div className="px-2 py-3 text-xs text-gray-400">{getTranslation('noConversations')}</div>
            )}
              ...
              {conversations.slice(0, 3).map((c) => (
                ...
                >
                  <div className="flex flex-col">
                    <span className="text-sm text-gray-100">{c.title || 'Chat'}</span>
                    <span className="text-xs text-gray-400">{c.messages?.length || 0} messages • {new Date(c.updatedAt || c.createdAt).toLocaleString()}</span>
                  </div>
                  <button onClick={(e) => { e.stopPropagation(); handleDeleteConversation(c.id); }} className="text-xs text-red-400 hover:text-red-200 ml-3 px-2 py-1 rounded">Delete</button>
                </div>
              ))}

              {conversations.length > 3 && (
                <div className="px-2 py-2">
                  <button onClick={() => setShowMore(!showMore)} className="w-full text-left px-2 py-2 rounded-md bg-slate-800 text-sm text-gray-200 hover:bg-slate-700/50">{showMore ? 'Hide' : `More (${conversations.length - 3})`}</button>
                  {showMore && (
                    <div className="mt-2 space-y-1">
                      {conversations.slice(3).map((c) => (
                        ...
                        >
                          <div className="flex flex-col">
                            <span className="text-sm text-gray-100">{c.title || 'Chat'}</span>
                            <span className="text-xs text-gray-400">{c.messages?.length || 0} • {new Date(c.updatedAt || c.createdAt).toLocaleString()}</span>
                          </div>
                          <button onClick={(e) => { e.stopPropagation(); handleDeleteConversation(c.id); }} className="text-xs text-red-400 hover:text-red-200 ml-3 px-2 py-1 rounded">Delete</button>
                        </div>
                      ))}
      ...
      <div className={`mt-4 space-y-3 border-t border-gray-700 pt-4 ${isCollapsed ? 'flex justify-center' : ''}`}>
        {!isCollapsed && (
          <div className="px-2 py-2">
            ...
            <p className="text-sm text-gray-300 truncate">{user ? user.emailAddresses[0]?.emailAddress : localStorage.getItem('guest_name') || 'Guest'}</p>
          </div>
        )}
        <button 
